var picturesData = require("../../data/pictures.js");
...
Page({
  ...
  onLoad: function () {
    ...
    wx.cloud.callFunction({
      ...
      data: { type: "getQuizData", collection: "quiz_pictures" },
  ...
  startQuiz: function () {
    ...
    if (this.cloudPool && this.cloudPool.length > 0) {
      ...
      questions = pool.map(function (item) {
        return { id: item.id, scene: item.scene, answer: item.answer, options: this.shuffle(item.options.slice()) };
      }.bind(this));
    } else {
      questions = picturesData.generateQuiz(8);
    }
  ...
  selectOption: function (event) {
    ...
    if (correct) {
      speaker.speakTTS(this.data.current.answer);
    }
  },
